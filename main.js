...
Promise.all([
  ...
  anchor = getAnchor()
  if (anchor!=null){
    var idanchor = decodeURIComponent(anchor)-1
    console.log(idanchor)
    console.log(data.nodes[decodeURIComponent(idanchor)])
    showLocomotionModal(data.nodes[decodeURIComponent(idanchor)].id,data.nodes[decodeURIComponent(idanchor)],defaultAttrConfig)
  }
